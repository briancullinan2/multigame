...
## TODO
  ...
  * TODO: add a feature where players can respawn as active bots with targets, and can teleport into the bot position, play as the bot, then switch to a different bot closer to the action. the bot takes over then the player finishes
  * TODO: add an automap feature that shows and exposed mini-map in the corner as a powerup like the one from DOOM or urban terror
