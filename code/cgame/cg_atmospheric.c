...
void CG_EffectParse( const char *effectstr ) {
	...
	if( CG_AtmosphericKludge() ) {
		CG_Printf( "Faking atmospheric effects for map.\n" );
		return;
	}

	trap_Cvar_Set("cg_atmosphere", effectstr); // ignored, for debugging
	...
	if( atmFXType == ATM_NONE || !BG_LoadTraceMap( cgs.rawmapname, cg.mapcoordsMins, cg.mapcoordsMaxs ) ) {
		// No effects
		cg_atmFx.numDrops = -1;
		CG_Printf("Missing a tracemap for %s.\n", cgs.rawmapname);
		return;
	}
...
#ifdef API_ET
// Keeger  copied from cg_view.c in ETMain
