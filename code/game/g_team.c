...
static int Team_TouchOurFlag( gentity_t *ent, gentity_t *other, team_t team ) {
	...
	// flag, he's just won!
	if (!other->client->ps.powerups[enemy_flag])
	...
	other->client->ps.powerups[enemy_flag] = 0;
...
static int Team_TouchEnemyFlag( gentity_t *ent, gentity_t *other, team_t team ) {
	...
	if( g_gametype.integer == GT_1FCTF ) {
		...
		other->client->ps.powerups[PW_NEUTRALFLAG] = INT_MAX; // flags never expire
		...
		if (team == TEAM_RED)
			other->client->ps.powerups[PW_REDFLAG] = INT_MAX; // flags never expire
		else
			other->client->ps.powerups[PW_BLUEFLAG] = INT_MAX; // flags never expire
