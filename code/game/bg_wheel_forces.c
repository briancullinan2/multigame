...
//	power = torque * car->rpm / (30 * 550 / M_PI); // hp
	...
	if (pm->ps->powerups[PW_HASTE] > 0){ // TODO: PW_TURBO
		torque *= 4.5f;
...
void PM_AddRoadForces(car_t *car, carBody_t *body, carPoint_t *points, float sec){
	...
	if (pm->ps->stats[STAT_HEALTH] > 0){
		...
		if (pm->ps->pm_type == PM_VEHICLEMOUSE){
			car->wheelAngle = WheelAngle(pm->ps->viewangles[YAW], pm->damageAngles[YAW]);
...
*/
			...
			pm->damageAngles[PITCH] = 0.0f;
			pm->damageAngles[YAW] = car->wheelAngle;

			pm->ps->damagePitch = ANGLE2BYTE(pm->damageAngles[PITCH]);
			pm->ps->damageYaw = ANGLE2BYTE(pm->damageAngles[YAW]);
		}
	}
	else {
		car->throttle = 0.0f;
	...
	if (car->gear < 0)
		pm->ps->eFlags |= EF_REVERSE;
	else
		pm->ps->eFlags &= ~EF_REVERSE;

	if (car->throttle < 0)
		pm->ps->eFlags |= EF_BRAKE;
	else
		pm->ps->eFlags &= ~EF_BRAKE;
